...
class ImportPDF
  ...
  def paragraphs
    ...
    reader.pages.each do |page|
      ...
      (0..lines.size - 2 - 1).each do |i|
        line = lines[i]
        next_line = lines[i + 1]

        full_sentence << line.gsub(/^\s+|\s+$/m, '')

        unless next_line[/\A */].size == 6
          # Process the full sentence
          full_sentence = full_sentence.join(' ').to_s
          # puts '---toan bo', full_sentence
          @paragraphs << full_sentence
          full_sentence = []
        end
      end
    end
